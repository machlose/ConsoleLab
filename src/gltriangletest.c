...
init_opengl(HDC real_dc)
{
    ...
    // Używamy kompatybilnego profilu żeby działał glBegin/glEnd
    int gl33_attribs[] = {
        WGL_CONTEXT_MAJOR_VERSION_ARB, 3,
        WGL_CONTEXT_MINOR_VERSION_ARB, 3,
        WGL_CONTEXT_PROFILE_MASK_ARB,  0x00000002, // WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB
        0,
    };
...
create_window(HINSTANCE inst)
{
    ...
    HWND window = CreateWindowExA(
        0, window_class.lpszClassName, "OpenGL - Test trójkąta RGB",
...
// -------------------------------------------------------
//  RENDERING - obracający się trójkąt RGB
// -------------------------------------------------------
...
render(void)
{
    ...
    // Macierz widoku - resetujemy i obracamy
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    glRotatef(angle, 0.0f, 0.0f, 1.0f);   // obrót wokół osi Z

    // Rysujemy trójkąt z kolorowymi wierzchołkami
    glBegin(GL_TRIANGLES);
        ...
        glColor3f(0.0f, 0.4f, 1.0f);   // niebieski
        glVertex2f( 0.6f, -0.4f);
    glEnd();
